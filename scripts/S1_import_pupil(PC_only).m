...
%% Replace ellipses with NaN in the ASCII files
ascii_files = dir(fullfile(ascdir, '**/*.asc')); % Find all .asc files

for i = 1:length(ascii_files)
    file_path = fullfile(ascii_files(i).folder, ascii_files(i).name);

    % Read the content of the file
    file_contents = fileread(file_path);

    % Replace ellipses with NaN
    file_contents = strrep(file_contents, '...', 'NaN');
    file_contents = strrep(file_contents, '.   .', 'NaN');  % Special case for periods with spaces

    % Write the modified content back to the file
    fid = fopen(file_path, 'w');
    if fid == -1
        disp(['Error opening file for writing: ', file_path]);
        continue;
    end
    fprintf(fid, '%s', file_contents);
    fclose(fid);

    disp(['Replaced ellipses with NaN in: ', ascii_files(i).name]);
end